app.controller('sideEffectCreate', ['$scope', '$http', '$rootScope', '$compile', function($scope, $http, $rootScope, $compile) {
    $scope.title = 'Tambah Efek Samping';
    $scope.formData = {}
    var path = window.location.pathname;
    if(/edit/.test(path)) {
        $scope.title = 'Edit Efek Samping';
        id = path.replace(/.+\/(\d+)/, '$1');
        $http.get(baseUrl + '/controller/user/side_effect/' + id).then(function(data) {
            $scope.formData = data.data
            detail_datatable.rows.add($scope.formData.detail).draw()
        }, function(error) {
  ...
  changeName = function(obj) {
    ...
    $scope.submitForm=function() {
      ...
      var url = baseUrl + '/controller/user/side_effect';
      var method = 'post';
      if($scope.formData.id) {
          var url = baseUrl + '/controller/user/side_effect/' + id;
          var method = 'put';
      } 
      ...
      $http[method](url, $scope.formData).then(function(data) {
        ...
        setTimeout(function () {
          window.location = baseUrl + '/side_effect'          
        }, 1000)
      }, function(error) {
        $rootScope.disBtn=false;
        if (error.status==422) {
