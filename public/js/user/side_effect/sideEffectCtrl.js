app.controller('sideEffect', ['$scope', '$compile', '$http', '$rootScope', function($scope, $compile, $http, $rootScope) {
  $scope.settingData = {}
  $scope.data = {
      grids : [
        {id : 1, name : 1},
  ...
  oTable = $('#listview').DataTable({
    ...
    dom: 'Blfrtip',
    ajax: {
      url : baseUrl+'/datatable/user/side_effect',
    ...
    buttons: [
      {
        ...
        'filename' : 'Efek Samping - '+new Date(),
        'sheetName' : 'Data',
        'title' : 'Efek Samping'
      },
    ],
    ...
    columns:[
      ...
      },
      {
        ...
        "<div class='btn-group'>" + 
        ( 
          resp.is_active == 1 ? "<button allow_destroy_sideEffect class='btn btn-xs btn-danger' ng-click='delete(" + resp.id + ")' title='Non-aktifkan'><i class='fa fa-trash-o'></i></button>"
          : "<button class='btn btn-xs btn-primary' allow_activate_sideEffect ng-click='activate(" + resp.id + ")' title='Aktifkan'><i class='fa fa-check'></i></button>"
        ) +
        "<a class='btn btn-xs btn-success' allow_edit_sideEffect href='" + baseUrl + "/side_effect/edit/" + resp.id +  "' title='Edit'><i class='fa fa-pencil'></i></a><a class='btn btn-xs btn-default' allow_show_sideEffect href='" + baseUrl + "/side_effect/" + resp.id +  "' title='Detail'><i class='fa fa-file-text-o'></i></a></div>"
      },
    ],
