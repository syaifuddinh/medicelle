app.controller('cutiHamilShow', ['$scope', '$http', '$rootScope', function($scope, $http, $rootScope) {
    $scope.title = 'Detail Surat Cuti Hamil';
    $scope.formData = {}
    ...
    $scope.show = function() {
        ...
        if(true) {
            ...
            $http.get(baseUrl + '/controller/letter/cuti_hamil/' + id).then(function(data) {
                ...
                $('#pdfDocument').attr('src', baseUrl + '/controller/letter/cuti_hamil/' + data.data.id + '/pdf')
                setTimeout(function () {    
    ...
    is_delete = confirm('Apakah anda ingin menon-aktifkan data ini ?');
      if(is_delete)
          $http.delete(baseUrl + '/controller/user/cutiHamil/' + id).then(function(data) {
