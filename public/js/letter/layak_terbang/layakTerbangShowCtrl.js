app.controller('layakTerbangShow', ['$scope', '$http', '$rootScope', function($scope, $http, $rootScope) {
    $scope.title = 'Detail Surat Layak Terbang';
    $scope.formData = {}
    ...
    $scope.show = function() {
        ...
        if(true) {
            ...
            $http.get(baseUrl + '/controller/letter/layak_terbang/' + id).then(function(data) {
                ...
                $('#pdfDocument').attr('src', baseUrl + '/controller/letter/layak_terbang/' + data.data.id + '/pdf')
                setTimeout(function () {    
    ...
    is_delete = confirm('Apakah anda ingin menon-aktifkan data ini ?');
      if(is_delete)
          $http.delete(baseUrl + '/controller/user/layakTerbang/' + id).then(function(data) {
    ...
    is_activate = confirm('Apakah anda ingin mengaktifkan data ini ?');
      if(is_activate)
          $http.put(baseUrl + '/controller/user/layakTerbang/activate/' + id).then(function(data) {
