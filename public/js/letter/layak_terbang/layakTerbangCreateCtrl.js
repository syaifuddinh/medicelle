app.controller('layakTerbangCreate', ['$scope', '$http', '$rootScope', '$filter', '$compile', function($scope, $http, $rootScope, $filter, $compile) {
    $scope.title = 'Tambah Surat Layak Terbang';
    $scope.formData = {
      age_type : 'MINGGU',
      additional : {}
    }
    ...
    $scope.show = function() {
        ...
        if(/edit/.test(path)) {
            $scope.title = 'Edit Surat Layak Terbang';
            id = path.replace(/.+\/(\d+)/, '$1');
            $http.get(baseUrl + '/controller/letter/layak_terbang/' + id).then(function(data) {
    ...
    $scope.submitForm=function() {
      ...
      var url = baseUrl + '/controller/letter/layak_terbang';
      var method = 'post';
      if($scope.formData.id) {
          var url = baseUrl + '/controller/letter/layak_terbang/' + id;
          var method = 'put';
      } 
      $http[method](url, $scope.formData).then(function(data) {
        ...
        if($scope.repeat == 1) {
            if(!id) {
                ...
                window.location = baseUrl + '/surat/layak_terbang/create'          
            }
        } else {
            setTimeout(function () {

              window.location = baseUrl + '/surat/layak_terbang'          
            }, 1000)
        }
      }, function(error) {
        $rootScope.disBtn=false;
