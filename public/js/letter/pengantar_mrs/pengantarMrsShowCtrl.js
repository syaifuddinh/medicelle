app.controller('pengantarMrsShow', ['$scope', '$http', '$rootScope', '$filter', function($scope, $http, $rootScope, $filter) {
    $scope.title = 'Detail Surat Pengantar MRS';
    $scope.formData = {}
    ...
    $scope.show = function() {
        ...
        if(true) {
            ...
            $http.get(baseUrl + '/controller/letter/pengantar_mrs/' + id).then(function(data) {
                ...
                $('#pdfDocument').attr('src', baseUrl + '/controller/letter/pengantar_mrs/' + data.data.id + '/pdf')
                setTimeout(function () {    
    ...
    is_delete = confirm('Apakah anda ingin menon-aktifkan data ini ?');
      if(is_delete)
          $http.delete(baseUrl + '/controller/user/pengantarMrs/' + id).then(function(data) {
