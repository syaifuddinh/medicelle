app.controller('pengantarMrsCreate', ['$scope', '$http', '$rootScope', '$filter', '$compile', function($scope, $http, $rootScope, $filter, $compile) {
    $scope.title = 'Tambah Surat Pengantar MRS';
    $scope.formData = {
      age_type : 'MINGGU',
      additional : {}
    }
    ...
    $scope.show = function() {
        ...
        if(/edit/.test(path)) {
            $scope.title = 'Edit Surat Pengantar MRS';
            id = path.replace(/.+\/(\d+)/, '$1');
            $http.get(baseUrl + '/controller/letter/pengantar_mrs/' + id).then(function(data) {
    ...
    $scope.submitForm=function() {
      ...
      var url = baseUrl + '/controller/letter/pengantar_mrs';
      var method = 'post';
      if($scope.formData.id) {
          var url = baseUrl + '/controller/letter/pengantar_mrs/' + id;
          var method = 'put';
      } 
      $http[method](url, $scope.formData).then(function(data) {
        ...
        if($scope.repeat == 1) {
            if(!id) {
                ...
                window.location = baseUrl + '/surat/pengantar_mrs/create'          
            }
        } else {
            setTimeout(function () {

              window.location = baseUrl + '/surat/pengantar_mrs'          
            }, 1000)
        }
      }, function(error) {
        $rootScope.disBtn=false;
