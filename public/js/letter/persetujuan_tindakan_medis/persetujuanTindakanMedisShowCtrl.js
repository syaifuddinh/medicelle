app.controller('persetujuanTindakanMedisShow', ['$scope', '$http', '$rootScope', function($scope, $http, $rootScope) {
    $scope.title = 'Detail Persetujuan Tindakan Medis';
    $scope.formData = {}
    ...
    $scope.show = function() {
        ...
        if(true) {
            ...
            $http.get(baseUrl + '/controller/letter/persetujuan_tindakan_medis/' + id).then(function(data) {
                ...
                $('#pdfDocument').attr('src', baseUrl + '/controller/letter/persetujuan_tindakan_medis/' + data.data.id + '/pdf')
                setTimeout(function () {    
    ...
    is_delete = confirm('Apakah anda ingin menon-aktifkan data ini ?');
      if(is_delete)
          $http.delete(baseUrl + '/controller/letter/persetujuan_tindakan_pasien/' + id).then(function(data) {
    ...
    is_activate = confirm('Apakah anda ingin mengaktifkan data ini ?');
      if(is_activate)
          $http.put(baseUrl + '/controller/letter/persetujuan_tindakan_pasien/activate/' + id).then(function(data) {
