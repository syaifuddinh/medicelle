app.controller('keteranganSehatCreate', ['$scope', '$http', '$rootScope', '$filter', '$compile', function($scope, $http, $rootScope, $filter, $compile) {
    $scope.title = 'Tambah Surat Keterangan Sehat';
    $scope.formData = {
      additional : {}
    }
    ...
    $scope.show = function() {
        ...
        if(/edit/.test(path)) {
            $scope.title = 'Edit Surat Keterangan Sehat';
            id = path.replace(/.+\/(\d+)/, '$1');
            $http.get(baseUrl + '/controller/letter/keterangan_sehat/' + id).then(function(data) {
    ...
    $scope.submitForm=function() {
      ...
      var url = baseUrl + '/controller/letter/keterangan_sehat';
      var method = 'post';
      if($scope.formData.id) {
          var url = baseUrl + '/controller/letter/keterangan_sehat/' + id;
          var method = 'put';
      } 
      $http[method](url, $scope.formData).then(function(data) {
        ...
        if($scope.repeat == 1) {
            if(!id) {
                ...
                window.location = baseUrl + '/surat/keterangan_sehat/create'          
            }
        } else {
            setTimeout(function () {

              window.location = baseUrl + '/surat/keterangan_sehat'          
            }, 1000)
        }
      }, function(error) {
        $rootScope.disBtn=false;
