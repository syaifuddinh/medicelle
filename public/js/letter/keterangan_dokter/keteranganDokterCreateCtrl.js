app.controller('keteranganDokterCreate', ['$scope', '$http', '$rootScope', '$filter', '$compile', function($scope, $http, $rootScope, $filter, $compile) {
    $scope.title = 'Tambah Surat Keterangan Dokter';
    $scope.formData = {
      age_type : 'MINGGU',
      duration_type : 'BULAN'
    }
    ...
    $scope.show = function() {
        ...
        if(/edit/.test(path)) {
            $scope.title = 'Edit Surat Keterangan Dokter';
            id = path.replace(/.+\/(\d+)/, '$1');
    ...
    $scope.submitForm=function() {
      ...
      var url = baseUrl + '/controller/letter/keterangan_dokter';
      var method = 'post';
      if($scope.formData.id) {
          var url = baseUrl + '/controller/letter/keterangan_dokter/' + id;
          var method = 'put';
      } 
      $http[method](url, $scope.formData).then(function(data) {
        ...
        if($scope.repeat == 1) {
            if(!id) {
                ...
                window.location = baseUrl + '/surat/keterangan_dokter/create'          
            }
        } else {
            setTimeout(function () {

              window.location = baseUrl + '/surat/keterangan_dokter'          
            }, 1000)
        }
      }, function(error) {
        $rootScope.disBtn=false;
