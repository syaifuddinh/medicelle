app.controller('rujukanPasienCreate', ['$scope', '$http', '$rootScope', '$filter', '$compile', function($scope, $http, $rootScope, $filter, $compile) {
    $scope.title = 'Tambah Surat rujukan pasien';
    $scope.formData = {
      additional : {}
    }
    ...
    $scope.show = function() {
        ...
        if(/edit/.test(path)) {
            $scope.title = 'Edit Surat rujukan pasien';
            id = path.replace(/.+\/(\d+)/, '$1');
            $http.get(baseUrl + '/controller/letter/rujukan_pasien/' + id).then(function(data) {
    ...
    $scope.submitForm=function() {
      ...
      var url = baseUrl + '/controller/letter/rujukan_pasien';
      var method = 'post';
      if($scope.formData.id) {
          var url = baseUrl + '/controller/letter/rujukan_pasien/' + id;
          var method = 'put';
      } 
      $http[method](url, $scope.formData).then(function(data) {
        ...
        if($scope.repeat == 1) {
            if(!id) {
                ...
                window.location = baseUrl + '/surat/rujukan_pasien/create'          
            }
        } else {
            setTimeout(function () {

              window.location = baseUrl + '/surat/rujukan_pasien'          
            }, 1000)
        }
      }, function(error) {
        $rootScope.disBtn=false;
