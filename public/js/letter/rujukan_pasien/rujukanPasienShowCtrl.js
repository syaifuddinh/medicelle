app.controller('rujukanPasienShow', ['$scope', '$http', '$rootScope', function($scope, $http, $rootScope) {
    $scope.title = 'Detail Surat rujukan pasien';
    $scope.formData = {}
    ...
    $scope.show = function() {
        ...
        if(true) {
            ...
            $http.get(baseUrl + '/controller/letter/rujukan_pasien/' + id).then(function(data) {
                ...
                $('#pdfDocument').attr('src', baseUrl + '/controller/letter/rujukan_pasien/' + data.data.id + '/pdf')
                setTimeout(function () {    
    ...
    is_delete = confirm('Apakah anda ingin menon-aktifkan data ini ?');
      if(is_delete)
          $http.delete(baseUrl + '/controller/user/rujukanPasien/' + id).then(function(data) {
