app.controller('radiologyCreate', ['$scope', '$http', '$rootScope', function($scope, $http, $rootScope) {
    ...
    $scope.show = function() {
        if(/edit/.test(path)) {
            ...
            id = path.replace(/.+\/(\d+)/, '$1');
            $http.get(baseUrl + '/controller/master/radiology/' + id).then(function(data) {
    ...
    $scope.category = function() {
        $http.get(baseUrl + '/controller/master/radiology/category/actived').then(function(data) {
    ...
    $scope.submitForm=function() {
      ...
      var url = baseUrl + '/controller/master/radiology';
      var method = 'post';
      if($scope.formData.id) {
          var url = baseUrl + '/controller/master/radiology/' + id;
          var method = 'put';
      } 
      $http[method](url, $scope.formData).then(function(data) {
        ...
        setTimeout(function () {
          window.location = baseUrl + '/radiology'          
        }, 1000)
      }, function(error) {
        $rootScope.disBtn=false;
        if (error.status==422) {
