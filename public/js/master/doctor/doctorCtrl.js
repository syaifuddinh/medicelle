app.controller('doctor', ['$scope', '$compile', '$http', function($scope, $compile, $http) {
  $scope.data = {}
  oTable = $('#listview').DataTable({
    ...
    columns:[
      ...
      },
      {
        ...
        "<div class='btn-group'>" + 
        ( 
          resp.is_active == 1 ? "<button class='btn btn-xs btn-danger' allow_destroy_medical_worker ng-click='delete(" + resp.id + ")' title='Non-aktifkan'><i class='fa fa-trash-o'></i></button>"
        ...
        "<a allow_edit_medical_worker class='btn btn-xs btn-success' href='" + baseUrl + "/doctor/edit/" + resp.id +  "' title='Edit'><i class='fa fa-pencil'></i></a><a allow_show_medical_worker class='btn btn-xs btn-default' href='" + baseUrl + "/doctor/" + resp.id +  "' title='Detail'><i class='fa fa-file-text-o'></i></a></div>"
      },
    ],
