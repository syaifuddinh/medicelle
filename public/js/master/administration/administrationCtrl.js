app.controller('administration', ['$scope', '$compile', '$http', function($scope, $compile, $http) {
  $scope.formData = {}
  oTable = $('#listview').DataTable({
    ...
    dom: 'Blfrtip',
    ajax: {
      url : baseUrl+'/datatable/master/administration',
    ...
    buttons: [
      {
        ...
        'filename' : 'Administrasi & Item Tindakan - '+new Date(),
        'sheetName' : 'Data',
        'title' : 'Administrasi & Item Tindakan'
      },
    ],
    ...
    columns:[
      ...
      {data:"administration_category.name", name:"administration_category.name"},
      ...
      },
      {
        ...
        "<a class='btn btn-xs btn-success' href='" + baseUrl + "/administration/edit/" + resp.id +  "' title='Edit'><i class='fa fa-pencil'></i></a><a class='btn btn-xs btn-default' href='" + baseUrl + "/administration/" + resp.id +  "' title='Detail'><i class='fa fa-file-text-o'></i></a></div>"
      },
    ],
  ...
  $scope.delete = function(id) {
    ...
    if(is_delete)
        $http.delete(baseUrl + '/controller/master/administration/' + id).then(function(data) {
            oTable.ajax.reload();
  ...
  $scope.activate = function(id) {
    is_activate = confirm('Apakah anda ingin mengaktifkan data ini ?');
      if(is_activate)
          $http.put(baseUrl + '/controller/master/administration/activate/' + id).then(function(data) {
