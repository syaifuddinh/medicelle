app.controller('administrationCreate', ['$scope', '$http', '$rootScope', function($scope, $http, $rootScope) {
    $scope.title = 'Tambah Administrasi & Item Tindakan';
    $scope.formData = {
        is_category : 0
    }
    $scope.data = {}
    ...
    $scope.show = function() {
        if(/edit/.test(path)) {
            $scope.title = 'Edit Administrasi & Item Tindakan';
            id = path.replace(/.+\/(\d+)/, '$1');
            $http.get(baseUrl + '/controller/master/administration/' + id).then(function(data) {
    ...
    $scope.category = function() {
        $http.get(baseUrl + '/controller/master/administration/category/actived').then(function(data) {
    ...
    $scope.submitForm=function() {
      ...
      var url = baseUrl + '/controller/master/administration';
      var method = 'post';
      if($scope.formData.id) {
          var url = baseUrl + '/controller/master/administration/' + id;
          var method = 'put';
      } 
      $http[method](url, $scope.formData).then(function(data) {
        ...
        setTimeout(function () {
          window.location = baseUrl + '/administration'          
        }, 1000)
      }, function(error) {
        $rootScope.disBtn=false;
        if (error.status==422) {
